...
class _AuthScreenState extends State<AuthScreen>
  ...
  Future<void> _restoreSession() async {
    ...
    final complete = await _progressStore.isComplete(user.uid);
    if (!mounted) return;
    _goTo(complete ? 6 : 2);
  }
  ...
  Future<void> _runAuthentication(
    ...
    try {
      ...
      final usesPassword = credential.user?.providerData.any(
            (provider) => provider.providerId == 'password',
          ) ??
          false;
      ...
      final complete = user != null && await _progressStore.isComplete(user.uid);
      if (!mounted) return;
      _goTo(complete ? 6 : 2);
  ...
  Future<void> _checkVerification() async {
    ...
    try {
      ...
      if (verified) {
        ...
        _showAuthError('Email is not verified yet. Open the link and try again.');
      }
    } on FirebaseAuthException catch (error) {
      if (mounted) _showAuthError(_authMessage(error));
    } finally {
                          ...
                          child: PageView(
                            ...
                            children: [
                              ...
                              _AccountTypePage(
                                ...
                                onConsumer: () => setState(() {
                                  if (_isConsumer && _sellerType == null) return;
                                  _isConsumer = !_isConsumer;
                                }),
                                onSeller: (value) => setState(() {
                                  _sellerType =
                                      _sellerType == value ? null : value;
                                  if (_sellerType == null) _isConsumer = true;
                                }),
...
class _Cloud extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Opacity(
        ...
        child: Icon(
          ...
          color: Colors.white,
          shadows: const [
            Shadow(color: Color(0x11000000), blurRadius: 8),
          ],
        ),
      );
}
...
class _WelcomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => _ScrollPage(
        child: Column(
          ...
          children: [
            const Text('Fresh food near you',
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(isSeller ? 'Create your public profile' : 'Tell us about you',
                style: _title),
            const SizedBox(height: 6),
            Text(
              isSeller
...
class _BusinessPage extends StatelessWidget {
  const _BusinessPage({required this.formKey, required this.onContinue});
  final GlobalKey<FormState> formKey;
  final VoidCallback onContinue;
  ...
  Widget build(BuildContext context) => _ScrollPage(
        child: Form(
          ...
          child: Column(
            ...
            const SizedBox(height: 18),
            _RequiredField(label: 'Business name'),
            const SizedBox(height: 12),
            TextFormField(decoration: const InputDecoration(labelText: 'Farm name')),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(child: _RequiredField(label: 'Business ID')),
            ...
            const SizedBox(height: 12),
            _RequiredField(label: 'Business address'),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(child: _RequiredField(label: 'City')),
                const SizedBox(width: 10),
                Expanded(child: _RequiredField(label: 'Zip code')),
              ],
            ),
            const SizedBox(height: 20),
...
class _ReviewPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final title = consumer && type != _AccountType.consumer
    ...
    return _ScrollPage(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 6),
          const Text('Review your setup before creating your account.',
              style: TextStyle(color: _muted)),
          ...
          const SizedBox(height: 22),
          _PrimaryButton(label: 'Create my account', onPressed: onFinish),
          const SizedBox(height: 10),
          const Text(
  ...
  Widget build(BuildContext context) => _PagePadding(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            const Text('Welcome to FRSH nearby!',
                textAlign: TextAlign.center, style: _title),
            const SizedBox(height: 8),
            Text(
            ...
            Stack(
              ...
              children: [
                CircleAvatar(
                  ...
                  child: photoBytes == null && photoUrl == null
                      ? const Icon(
